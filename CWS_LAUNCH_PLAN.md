...
1. **BIZ** — 아래 세 안 중 하나를 고른다(문장은 그대로 쓸 수 있게 써 두었다).
2. **EXT+SITES** — 고른 안의 "코드에서 해야 하는 일"을 실행한다.
3. **BIZ** — 데이터 수집 항목을 그 목적 기준으로 재정리(P0-4).

**완료 판정** 단일 목적 문장 하나와, 각 플로우가 그 안에 있는지 없는지의 표.

---

#### 초안 (2026-08-15, SITES 작성) — 세 안, 실측 첨부

판단의 축은 취향이 아니라 **§1의 "묶음"과 Limited Use의 "엄격히 필요"를 동시에 통과하는가** 하나다.
아래 바이트는 flows 문서가 이름으로 부르는 모듈을 디스크에서 실측한 값이다(전체 **25개 · 371.4 KiB**).

| 안 | 문장 | flow | 모듈 | KiB | 덜어냄 |
|---|---|---|---|---|---|
| **A** | *"지원 쇼핑몰에서 한 상품의 배송비 포함 총액을 비교하고, 사용자가 고른 상품을 장바구니에 넣고 결제 화면까지 안내한다."* | 4 | 20 | **282.1** | **89.3 KiB (24%)** |
| **B** | *"온라인 쇼핑과 지역 서비스 견적에서, 사용자를 대신해 조건을 비교하고 요청을 대행한다."* | 6 | 24 | 367.3 | 4.1 KiB (1.1%) |
| **C** | 현재 상태 (문장 없음) | 7 | 25 | 371.4 | — |

**각 플로우가 안에 있는지 없는지**

| flow | 변경 | A | B | C |
|---|---|---|---|---|
| `shopping_multi_store_total_cost` | 읽기 | ✅ | ✅ | ✅ |
| `shopping_single_site` | **장바구니** | ✅ | ✅ | ✅ |
| `checkout` | 읽기(주문 안 함) | ✅ | ✅ | ✅ |
| `end_conversation` | 없음 | ✅ | ✅ | ✅ |
...
1. **메모리의 정당성은 독립 선택이 아니라 견적 플로우의 종속이다.** `70_rpc_memory`에 닿는 것은
   `request_service_quote`(연락처 재사용 recall)와 `memory` 둘뿐이다. 견적이 출시되면 이름·이메일·전화는
   견적 폼이 **실제로 요구하는** 값이므로 "엄격히 필요"가 성립한다. 견적이 빠지면 성립할 근거가 없다 —
   상품 비교는 사용자의 전화번호를 필요로 하지 않는다.
2. **A는 훅도 함께 지워야 한다.** `hooks.beforeIntent: [record_memory]`는 라우팅과 무관하게 **매 턴** 돌고
   `70_rpc_memory`에 닿는다. 즉 `memory` intent만 빼면 메모리 *기능*은 사라지지만 메모리 *수집*은 남는다.
   목적이 "상품 구매"인데 매 턴 연락처를 추출하는 훅이 살아 있으면 Limited Use에서 방어 불가다.
   **A를 고르면 훅 삭제는 리스팅 작업이 아니라 코드 작업이다.**
3. **`router.defaultIntent`가 지금 `request_service_quote`다.** 매칭 안 된 발화가 **Thumbtack 견적 플로우로
   들어간다.** A·B 어느 쪽이든(A는 그 플로우가 없어서, B는 목적 문장의 앞부분이 쇼핑이라서) 이 기본값은
   쇼핑 플로우로 옮겨야 한다. 심사자가 아무 말이나 입력했을 때 리스팅과 다른 답이 나오는 것이 §1 위반의
   가장 흔한 발견 경로다.

**안별로 코드에서 해야 하는 일**

- **A** — 라우터에서 3개 intent 제거 · `defaultIntent` 이전 · **캡처 훅 삭제** · 전용 모듈 4개
  (`64_rpc_thumbtack`, `65_rpc_quote`, `10_form_wizard`, `72_rpc_sitemap`)와 `70_rpc_memory` 제거 ·
  `thumbtack/`·`bluemoonsoft/` 사이트 데이터를 스토어 빌드에서 제외. 게이트가 잡아준다(`dead:lua`,
  `check:flows`, `build:schema --check`).
- **B** — `bluemoonsoft`만 제거(전용 `72_rpc_sitemap` 4.1 KiB) · `defaultIntent` 이전 · 견적의 연락처
  수집을 공시에 명시. **§1 리스크는 A보다 높다**: 심사자에게 소매 상품과 지역 서비스 리드는 다른 버티컬로
  보일 수 있다.
...
요인까지 같이 줄어든다. 견적 플로우는 **버리는 것이 아니라** 두 번째 확장으로 분리할 수 있다 — §1의
처방이 바로 그것이고, 코드는 이미 모듈 단위로 갈라져 있다(전용 75.5 KiB, 공유 모듈에 대한 역의존 없음).
...
**차단 요인은 이제 세 건이다 — P0-3 · P0-4 · P0-5, 그리고 셋 다 BIZ 결정이다.** P0-1은 **완료**(§M1: 스토어를
비운 뒤 하네스 쓰기 없이 확장이 스스로 채우고, `provision: false`로 전체 비교가 한 턴에 답했다). 남은 셋은
서로 묶여 있어 순서를 고를 수 없다: **P0-3(단일 목적)이 어떤 플로우를 출시에 포함할지 정하고**, 그 답이
P0-4(Limited Use·공시 범위)를 정하며, P0-5(릴리스 모델)의 답이 "심사 지연 수용 불가"로 나오면 **P0-1의 데이터
분리 범위를 다시 설계해야 한다** — 즉 완료된 M1이 다시 열릴 수 있는 유일한 경로가 P0-5다.

**엔지니어링 쪽에서 이 셋을 앞당길 방법은 없다.** 문장 하나(P0-3)가 나오면 M4 → M5가 순차로 풀린다.
...
| 자산 | 성격 | 배달 |
|---|---|---|
| flows가 선언한 `_common/rpc` 14개 + `_common/scripts` 12개 (370.7 KiB) | 로직 | **번들 필수** |
| flows 문서 (208.5 KiB) | 로직 — flowTool마다 `execute.lua`가 들어 있다 | **번들 필수** |
| 사이트 선언 `<site>/scripts/*.lua` (13개, 76.8 KiB) | **순수 데이터 선언** — durable 삭제 후 `AX_SITE_CONFIGS[site] = CONFIG`뿐이고, `62_rpc_sites.lua`의 **생성원**이다 | 번들 불필요(생성물이 대신 간다). 이전 판의 "개발 시점 스택 109.3 KiB"는 durable 명령이 남아 있던 시절의 수치 |
| 사이트 인덱스, 사이트맵 | 데이터 | 원격 가능 |
| **선택자 테이블 `62_rpc_sites.lua`** (27.8 KiB · 936줄 · `function` 0 · 분기 0) | **순수 데이터** | **JSON으로 바꾸면** 원격 가능 **[확인필요]** |

원래 구상(백엔드에서 Lua 수신)은 **그대로는 불가능**하다(단, 부록 D의 M1-B는 예외 가능성). 다만 그
구상의 목적 — *상점이 마크업을 바꿔도 심사 없이 대응한다* — 은 **선택자만 데이터로 빼면 대부분
살아남는다.** 비율은 **번들 95 : 원격 데이터 5**(현재 실측 `build:bundle` 기준 579.2 KiB 중 27.8 KiB = 4.8%).
