...
**설계 논의는 4라운드로 닫혔다.** 이후 12·13차에서 R23~R27이 추가됐다(플랫폼 회신 12차 참조).

### 현재 남은 `kind: remote` 21개 선언 = 고유 15종

| 갈래 | 고유 종 | 선언 | 막는 것 |
|---|---:|---:|---|
| 메모리 4종 · `render_widget` · `sitemap_search` | 6 | 9 | **R25·R26·R27** (`RPC_LUA_RUNTIME_REQUESTS_13.md`) |
| 비교 스냅샷 `rank`/`present`/`refine` | 3 | 3 | 우리 설계 결정 — 세션 스코프 저장을 어디에 둘지 |
| `resolve_zip` | 1 | 1 | R1 `net.fetch` |
| `AX_navigate` | 1 | 1 | **아무것도. 우리 몫** — `nav.navigate` + `wait_for_navigation`으로 된다 |
| amazon 계열 `open_site`·`checkout`·`add_to_cart`·`add_store_product_to_cart` | 4 | 7 | 아무것도. 이식 미착수 |

13차 요청서를 쓰면서 **우리 분류가 하나 틀렸음을 확인했다**: `AX_navigate`를 오래 "플랫폼 소유"로
분류해 왔는데, 구현(`axhandler.ts:566`)은 URL 조립 + 도착 확인이고 둘 다 우리가 이미 쓰는 op이다.
